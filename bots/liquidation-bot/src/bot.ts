...
export class LiquidationBot {
    ...
    private async monitorAllLoans(): Promise<void> {
        ...
        try {
            ...
            const [price, priceTimestamp] = await this.oracle.getPrice(this.config.stRwaTokenAddress);
            ...
            for (const borrower of borrowers) {
                try {
                    const result = await this.monitorLoan(borrower, price, priceTimestamp);
                    results.push(result);
                } catch (error: any) {
                    this.logger.error('Failed to monitor loan', { borrower, error: error.message });
                    results.push({ borrower, success: false, error: error.message, healthy: false, healthFactor: 0 });
                }
            }
    ...
    public async monitorLoan(borrower: string, price: bigint, priceTimestamp: number): Promise<MonitoringResult> {
        const loan = await this.lendingPool.getLoan(borrower);
        if (!loan) {
            return { borrower, success: true, healthy: true, healthFactor: Infinity };
        }

        const health = await this.healthCalculator.calculateHealth(borrower, loan, price, priceTimestamp);
        this.metrics.recordHealthFactor(borrower, health.healthFactor);

        if (health.needsLiquidation) {
            const economics = await this.economicsCalculator.analyzeLiquidation(health);
            if (!economics.isProfitable) {
                ...
                return { borrower, success: true, healthy: false, healthFactor: health.healthFactor, skippedUnprofitable: true };
            }
            ...
            const liquidationResult = await this.liquidationExecutor.liquidateLoan(borrower);
            this.metrics.recordLiquidation(liquidationResult.success, liquidationResult.reward, economics.gasCost);
            return { ...liquidationResult, healthy: false, healthFactor: health.healthFactor, liquidated: true };
        }

        const currentTime = Date.now() / 1000;
        const warningState = this.warningManager.determineWarningState(health, loan, currentTime);
        if (this.warningManager.shouldIssueWarning(warningState, loan, currentTime)) {
            ...
            return { borrower, success: true, healthy: false, healthFactor: health.healthFactor, warningIssued: true, warningState, txHash };
        }

        return { borrower, success: true, healthy: true, healthFactor: health.healthFactor };
    }
