...
export class MetricsCollector {
  ...
  recordUpdate(
    ...
    assetMetrics.averageLatency =
      assetMetrics.successfulUpdates > 0
  ...
  recordSourceSuccess(asset: string, sourceName: string, responseTime: number) {
    ...
    sourceHealth.successRate =
      (sourceHealth.successfulRequests / sourceHealth.totalRequests) * 100;
    sourceHealth.averageResponseTime =
      (sourceHealth.averageResponseTime *
        (sourceHealth.successfulRequests - 1) +
        responseTime) /
      sourceHealth.successfulRequests;
  }
  ...
  checkHealth() {
    ...
    for (const [asset, metrics] of this.metrics.entries()) {
      ...
      if (staleness > 300) {
        // 5 minutes
        console.warn(
          `⚠️ Oracle update is stale for ${asset}: ${staleness}s old`,
        );
      }
    }
  }
